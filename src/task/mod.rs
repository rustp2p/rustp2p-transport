...
use rustp2p::pipe::{Pipe, PipeLine, PipeWriter};
use tcp_ip::ip_stack::{IpStackRecv, IpStackSend};
use bytes::BytesMut;

pub async fn start(mtu: usize, pipe: Pipe, ip_stack_send: IpStackSend, ip_stack_recv: IpStackRecv) -> io::Result<()> {
    ...
    tokio::spawn(async move{
        if let Err(e) = ip_stack_recv_handle(mtu, ip_stack_recv, pipe_writer).await {
            log::warn!("pipe_accept {e:?}");
        }
    });
    Ok(())
...
async fn ip_stack_recv_handle(mtu: usize, mut ip_stack_recv: IpStackRecv, pipe_writer: PipeWriter) -> rustp2p::error::Result<()> {
...
async fn pipe_accept_handle(mut pipe: Pipe, ip_stack_send: IpStackSend) -> rustp2p::error::Result<()> {
    loop {
...
async fn pipe_line_recv_handle(mut pipe_line: PipeLine, ip_stack_send: IpStackSend) -> rustp2p::error::Result<()> {
    let mut list = Vec::with_capacity(16);

    loop {
        let result = pipe_line.recv_multi(&mut list).await?;
        if let Err(e) = result {
